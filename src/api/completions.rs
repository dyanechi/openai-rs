...
pub trait CompletionsApi {
    fn completions_create(&self, completions_body: &CompletionsBody) -> ApiResult<Completion>;
}

impl CompletionsApi for OpenAi {
    fn completions_create(&self, completions_body: &CompletionsBody) -> ApiResult<Completion> {
        let body = serde_json::to_value(completions_body).unwrap();
        let res = self.post(COMPLETION_CREATE, body)?;
